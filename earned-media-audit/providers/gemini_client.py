...
class GeminiProvider:
    """
    Provider using Google Gemini (Generative Language API) to produce a concise
    answer and a JSON citations object.
...
"""
    ...
    def _call_api(self, prompt: str) -> Optional[dict]:
        ...
        try:
            ...
            resp = requests.post(self.endpoint, params=params, json=payload, timeout=45)
    ...
    @staticmethod
    def _read_text_from_response(data: dict) -> str:
        try:
    ...
    def _parse_response(self, data: dict) -> Optional[List[Dict[str, Any]]]:
        if not data:
            return None
        try:
            text = self._read_text_from_response(data)
