...
class PerplexityProvider:
    """
    Provider that uses Perplexity's official API (chat/completions) to generate
    answer-style outputs and return JSON citations.
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "sonar"):
        ...
        # API endpoint as documented by Perplexity
...
"""
    ...
    def _parse_response(self, data: dict) -> Optional[List[Dict[str, Any]]]:
        ...
        # If Perplexity returns a top-level citations list (some models do), use it
        # Otherwise parse the assistant message text for our JSON object.
        try:
            # Direct citations (rare but supported by some responses)
